  ...
  <div class="pay-main">
    <div class="pay-container">
      <div class="checkout-tit">
        ...
        <div class="paymark">
          ...
          <span class="fr">
            ...
            <em class="orange money">￥{{orderPayInfo.totalFee}}</em>
          </span>
        </div>
      </div>
      <div class="checkout-info">
        <h4>重要说明：</h4>
        <h3>
          <a :href="orderPayInfo.codeUrl">支付地址</a>
        </h3>
        <ol>
          <li>
            尚品汇商城支付平台目前支持
...
export default {
  ...
  props: ["orderId"],
  mounted() {
    this.$store.dispatch("getOrderPayInfo", this.orderId);
  },
  computed: {
    ...mapState({
      orderPayInfo: state => state.order.orderPayInfo
    })
  },
  methods: {
    Pay() {
      QRCode.toDataURL(this.orderPayInfo.codeUrl)
        .then(imgUrl => {
          ...
          this.$alert(`<img src="${imgUrl}">`, "请使用微信扫码支付", {
            dangerouslyUseHTMLString: true, // 解析内容html标签
